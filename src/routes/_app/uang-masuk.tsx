...
export const Route = createFileRoute("/_app/uang-masuk")({
  component: UangMasukPage,
});
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 md:grid-cols-[1fr_180px_180px]">
        <SearchBar value={query} onChange={(v) => { setQuery(v); pager.setPage(1); }} placeholder="Cari sumber, kategori, atau keterangan" />
        ...
        {filtered.length === 0 ? (
          <div className="p-5">
            <EmptyState
              ...
              action={<Button onClick={() => setOpen(true)}>Tambah Uang Masuk</Button>}
            />
          </div>
        ) : (
          <>
      ...
      <Dialog open={Boolean(editing)} onOpenChange={(v) => !v && setEditing(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Edit Uang Masuk</DialogTitle>
            <DialogDescription>Perbarui data pemasukan yang dipilih.</DialogDescription>
          </DialogHeader>
          {editing ? (
            <IncomeForm
              initial={editing}
