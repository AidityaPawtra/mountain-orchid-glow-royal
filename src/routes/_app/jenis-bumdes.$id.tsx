...
export const Route = createFileRoute("/_app/jenis-bumdes/$id")({
...
function RouteComponent() {
  ...
  const bumdes = bumdesTypes.find((item) => item.id === id);
 ...
 const currentBumdes = bumdes;
  ...
  const bumdesIncome = income.filter(
    (item) => item.bumdesTypeId === bumdes.id,
  );

  const bumdesExpenses = expenses.filter(
    (item) => item.bumdesTypeId === bumdes.id,
  );

  const totalIncome = bumdesIncome.reduce(
    (total, item) => total + item.amount,
    0,
  );

  const totalExpense = bumdesExpenses.reduce(
    (total, item) => total + item.amount,
    0,
  );
