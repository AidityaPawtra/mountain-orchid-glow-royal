...
function JenisBumdesPage() {
  const bumdesTypes = useAppStore(
    (state) => state.bumdesTypes,
  );

  const addBumdesType = useAppStore(
    (state) => state.addBumdesType,
  );

  const updateBumdesType = useAppStore(
    (state) => state.updateBumdesType,
  );

  const deleteBumdesType = useAppStore(
    (state) => state.deleteBumdesType,
  );
            ...
            (item) => (
              ...
              >
                ...
                <p className="mt-5 min-h-[60px] text-sm leading-5 text-muted-foreground">
                  {item.description ||
                    "Tidak ada deskripsi untuk unit BUMDes ini."}
                </p>
